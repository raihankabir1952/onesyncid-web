...
export default function Page() {
  ...
  return (
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
              <p style={{ fontSize: 18, fontWeight: 600, color: "#000", marginTop: 0, marginBottom: 0 }}>Smart Support</p>
              <p style={{ fontSize: 14, color: "#5e5757", marginTop: 0, marginBottom: 0 }}>Usually answers in seconds</p>
            </div>
    ...
    >
      ...
      {/* Chat area — negative margin to go edge-to-edge */}
      <div style={{ flex: 1, backgroundColor: "rgba(185,185,185,0.05)", display: "flex", flexDirection: "column", justifyContent: "space-between", margin: "0 -30px -30px -30px" }}>
        ...
        {/* Messages — gap: 30 (Figma: gap-[30px]) */}
        <div style={{ flex: 1, overflowY: "auto", padding: "30px 20px 10px", display: "flex", flexDirection: "column", gap: 30 }}>
          {messages.map((msg) => (
            <div key={msg.id}>
              {msg.role === "user" ? (
                <div style={{ display: "flex", justifyContent: "flex-end" }}>
                  <div style={{ backgroundColor: "#025fc9", borderRadius: 12, padding: "8px 12px", maxWidth: "70%" }}>
                    <p style={{ fontSize: 16, color: "#fff", marginTop: 0, marginBottom: 0, lineHeight: "22.75px" }}>{msg.text}</p>
                  </div>
                </div>
              ) : (
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  ...
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    <div style={{ backgroundColor: "#fff", border: "1px solid #d9d9d9", borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 20 }}>
                      {msg.text && (
                        <p style={{ fontSize: 16, color: "#5e5757", marginTop: 0, marginBottom: 0, lineHeight: "22.75px" }}>{msg.text}</p>
                      )}
                      {msg.steps && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          {msg.steps.map((step) => (
                            <div key={step.title} style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                              <p style={{ fontSize: 16, fontWeight: 500, color: "#333", marginTop: 0, marginBottom: 0 }}>{step.title}</p>
                              <p style={{ fontSize: 14, color: "#5e5757", marginTop: 0, marginBottom: 0 }}>{step.desc}</p>
                            </div>
                          ))}
                        </div>
                      )}
                    ...
                    {msg.showAgent && (
                      <div style={{ backgroundColor: "#fff", border: "1px solid #d9d9d9", borderRadius: 12, overflow: "hidden" }}>
                        <div style={{ padding: 16, borderBottom: "1px solid #d9d9d9", display: "flex", flexDirection: "column", gap: 10 }}>
                          <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                            <p style={{ fontSize: 14, color: "#5e5757", marginTop: 0, marginBottom: 0, letterSpacing: "0.14px", lineHeight: "21px" }}>STILL NOT RESOLVED?</p>
                            <p style={{ fontSize: 16, fontWeight: 600, color: "#000", marginTop: 0, marginBottom: 0, lineHeight: "21px" }}>Talk to a human agent</p>
                          </div>
                          <button type="button" onClick={() => router.push("/support/agent")}
            ...
            <div style={{ flex: 1, height: 44, display: "flex", alignItems: "center", backgroundColor: "#fff", border: "1px solid #d9d9d9", borderRadius: 9999, padding: "8px 16px", gap: 8 }}>
              <input type="text" placeholder="Describe your issue..." value={message}
                ...
                style={{
                  ...
                  color: "#5e5757", // ← was #333, Figma: text-[#5e5757]
